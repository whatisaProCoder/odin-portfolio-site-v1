import { LogoSet } from "../constants/logoSet";

export const Projects = [
  {
    id: 0,
    title: "ShareFY",
    imageSrc: "https://raw.githubusercontent.com/whatisaProCoder/nodejs-file-uploader/refs/heads/main/public/dashboard.png",
    summary: "Secure file management app where users can organize files into folders, upload/download them and share them with time-limits.",
    repository: "https://github.com/whatisaProCoder/nodejs-file-uploader",
    live: "https://sharefy.onrender.com/",
    techUsed: [LogoSet.EJS, LogoSet.CSS, LogoSet.nodejs, LogoSet.ExpressJS, LogoSet.PostgreSQL]
  },
  {
    id: 1,
    title: "RXTX Clubhouse",
    imageSrc: "https://github.com/whatisaProCoder/rxtx-clubhouse/blob/main/public/showcase.png?raw=true",
    summary: "A clubhouse app showcasing authentication, role-based authorization, sessions, form validation, and secure password hashing.",
    repository: "https://github.com/whatisaProCoder/rxtx-clubhouse",
    live: "https://rxtx-clubhouse.onrender.com/",
    techUsed: [LogoSet.EJS, LogoSet.CSS, LogoSet.nodejs, LogoSet.ExpressJS, LogoSet.PostgreSQL]
  },
  {
    id: 2,
    title: "Robotics Inventory",
    imageSrc: "https://github.com/whatisaProCoder/robotics-inventory-website/blob/main/public/showcase.png?raw=true",
    summary: "Inventory Management System demonstrating Node.js, Express, PostgreSQL, CRUD operations, and server-side rendering with EJS.",
    repository: "https://github.com/whatisaProCoder/robotics-inventory-website",
    live: "https://robotics-inventory-website.onrender.com/",
    techUsed: [LogoSet.EJS, LogoSet.CSS, LogoSet.nodejs, LogoSet.ExpressJS, LogoSet.PostgreSQL]
  },
  {
    id: 3,
    title: "Odin Message Board",
    imageSrc: "https://github.com/whatisaProCoder/odin-message-board/blob/main/public/showcase.png?raw=true",
    summary: "A simple message board web application demonstrating Express, Routers, Controllers, EJS and MVC Architecture.",
    repository: "https://github.com/whatisaProCoder/odin-message-board",
    live: "https://odin-message-board-k8tp.onrender.com/",
    techUsed: [LogoSet.HTML, LogoSet.CSS, LogoSet.JavaScript, LogoSet.nodejs]
  },
  {
    id: 4,
    title: "Fakestore",
    imageSrc: "https://github.com/whatisaProCoder/odin-fakestore-site/blob/main/public/showcase.png?raw=true",
    summary: "An e-commerce web application built as part of The Odin Project's Shopping Cart assignment. Evolved into a comprehensive mock online store.",
    repository: "https://github.com/whatisaProCoder/odin-fakestore-site",
    live: "https://odin-fakestore-site.vercel.app/",
    techUsed: [LogoSet.React, LogoSet.TailwindCSS, LogoSet.JavaScript, LogoSet.vitejs]
  },
  {
    id: 5,
    title: "CV Generator",
    imageSrc: "https://github.com/whatisaProCoder/odin-cv-generator-app/blob/main/src/assets/images/showcase.png?raw=true",
    summary: "Lets you create professional CVs with real-time preview. You fill out the forms on the left, and your CV updates instantly on the right.",
    repository: "https://github.com/whatisaProCoder/odin-cv-generator-app",
    live: "https://odin-c-v-generator.vercel.app/",
    techUsed: [LogoSet.React, LogoSet.TailwindCSS, LogoSet.JavaScript, LogoSet.vitejs]
  },
  {
    id: 6,
    title: "AoT - Memory Game",
    imageSrc: "https://github.com/whatisaProCoder/aot-memory-game/blob/main/public/showcase.png?raw=true",
    summary: "A memory card game featuring AoT characters with three difficulty levels, where players must avoid clicking the same character twice.",
    repository: "https://github.com/whatisaProCoder/aot-memory-game",
    live: "https://aot-memory-game.vercel.app/",
    techUsed: [LogoSet.React, LogoSet.TailwindCSS, LogoSet.JavaScript, LogoSet.vitejs]
  },
  {
    id: 7,
    title: "Portfolio Website",
    imageSrc: "./showcase.png",
    summary: "My first dedicated portfolio website made in React, showcasing my interests, projects and tech stack in a custom-made creative Figma design.",
    repository: "https://github.com/whatisaProCoder/odin-portfolio-site-v1",
    live: "https://pritamdebnath.vercel.app",
    techUsed: [LogoSet.React, LogoSet.TailwindCSS, LogoSet.JavaScript, LogoSet.vitejs]
  },
  {
    id: 8,
    title: "Polymath",
    imageSrc: "https://github.com/whatisaProCoder/odin-task-management-app/blob/main/demo/app_preview.png?raw=true",
    summary: "An intuitive task management application built with just VanillaJS, no frameworks and packaged as a desktop application using Electron.",
    repository: "https://github.com/whatisaProCoder/odin-task-management-app",
    live: "https://whatisaprocoder.github.io/odin-task-management-app/",
    techUsed: [LogoSet.HTML, LogoSet.CSS, LogoSet.JavaScript, LogoSet.Webpack, LogoSet.Electron]
  },
  {
    id: 9,
    title: "Weather App",
    imageSrc: "https://github.com/whatisaProCoder/odin-weather-app/blob/main/demo/dashboard.png?raw=true",
    summary: "A sleek, responsive weather application that provides current weather data and weekly forecasts for any location around the world.",
    repository: "https://github.com/whatisaProCoder/odin-weather-app",
    live: "https://whatisaprocoder.github.io/odin-weather-app/",
    techUsed: [LogoSet.HTML, LogoSet.TailwindCSS, LogoSet.JavaScript, LogoSet.Webpack]
  },
  {
    id: 10,
    title: "Giphy App",
    imageSrc: "https://github.com/whatisaProCoder/odin-giphy-api-ui/raw/main/src/images/demo.gif",
    summary: "A simple application that lets you search for GIFs using the GIPHY API. It demonstrates how to fetch data from an external API.",
    repository: "https://github.com/whatisaProCoder/odin-giphy-api-ui",
    live: "https://whatisaprocoder.github.io/odin-giphy-api-ui/",
    techUsed: [LogoSet.HTML, LogoSet.TailwindCSS, LogoSet.JavaScript, LogoSet.Webpack]
  },
  {
    id: 11,
    title: "Custom Image Carousel",
    imageSrc: "https://github.com/whatisaProCoder/custom-image-carousel-component/blob/main/demo/demo.png?raw=true",
    summary: "A lightweight, easy-to-use npm package that provides image carousel component with navigation dots, arrow controls, and smooth scrolling.",
    repository: "https://github.com/whatisaProCoder/custom-image-carousel-component/",
    live: "https://whatisaprocoder.github.io/custom-image-carousel-component/",
    techUsed: [LogoSet.HTML, LogoSet.CSS, LogoSet.TailwindCSS, LogoSet.JavaScript, LogoSet.Webpack, LogoSet.npm]
  },
  {
    id: 12,
    title: "Custom Dropdown Menu",
    imageSrc: "https://github.com/whatisaProCoder/custom-dropdown-menu/blob/main/demo/recording.gif?raw=true",
    summary: "A beginner-friendly, lightweight npm package that provides dropdown menu component that's easy to use and customize.",
    repository: "https://github.com/whatisaProCoder/custom-dropdown-menu",
    live: "https://whatisaprocoder.github.io/custom-dropdown-menu/",
    techUsed: [LogoSet.HTML, LogoSet.CSS, LogoSet.TailwindCSS, LogoSet.JavaScript, LogoSet.Webpack, LogoSet.npm]
  },
  {
    id: 13,
    title: "Chillax Restaurant Website",
    imageSrc: "https://github.com/whatisaProCoder/odin-restaurant-site/blob/main/demo/showcase.png?raw=true",
    summary: "A modern restaurant website built as single-page application showcases a fictional restaurant with dynamic content loading.",
    repository: "https://github.com/whatisaProCoder/odin-restaurant-site",
    live: "https://whatisaprocoder.github.io/odin-restaurant-site/",
    techUsed: [LogoSet.HTML, LogoSet.CSS, LogoSet.JavaScript, LogoSet.Webpack]
  },
  {
    id: 14,
    title: "Tic Tac Toe",
    imageSrc: "https://github.com/whatisaProCoder/odin-tic-tac-toe-game/blob/main/demo/showcase.png?raw=true",
    summary: "A simple web-based Tic-Tac-Toe game built with HTML, CSS, and JavaScript as part of The Odin Project curriculum.",
    repository: "https://github.com/whatisaProCoder/odin-tic-tac-toe-game",
    live: "https://whatisaprocoder.github.io/odin-tic-tac-toe-game/",
    techUsed: [LogoSet.HTML, LogoSet.CSS, LogoSet.JavaScript]
  },
  {
    id: 15,
    title: "Library Application",
    imageSrc: "https://github.com/whatisaProCoder/odin-library-app/blob/main/demo/showcase.png?raw=true",
    summary: "A simple, responsive library web application that allows users to add, view, and manage books in a sleek, minimalist interface.",
    repository: "https://github.com/whatisaProCoder/odin-library-app",
    live: "https://whatisaprocoder.github.io/odin-library-app/",
    techUsed: [LogoSet.HTML, LogoSet.CSS, LogoSet.JavaScript]
  },
  {
    id: 16,
    title: "Admin Dashboard",
    imageSrc: "https://github.com/whatisaProCoder/odin-admin-dashboard/blob/main/demo/showcase.png?raw=true",
    summary: "Admin Dashboard for The Odin Project — Flexbox & Grid layout, minimalist design, light/dark toggle.",
    repository: "https://github.com/whatisaProCoder/odin-admin-dashboard",
    live: "https://whatisaprocoder.github.io/odin-admin-dashboard/",
    techUsed: [LogoSet.HTML, LogoSet.CSS, LogoSet.JavaScript]
  },
  {
    id: 17,
    title: "TOP - Sign Up Form",
    imageSrc: "https://github.com/whatisaProCoder/odin-signup-form/blob/main/demo/showcase.png?raw=true",
    summary: "This is the basic Sign-Up Form we get to build in the \"Intermediate HTML & CSS\" section of The Odin Project.",
    repository: "https://github.com/whatisaProCoder/odin-signup-form",
    live: "https://whatisaprocoder.github.io/odin-signup-form/",
    techUsed: [LogoSet.HTML, LogoSet.CSS]
  }
]